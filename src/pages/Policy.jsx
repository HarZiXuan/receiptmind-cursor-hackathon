  ...
  return (
    <div className="h-[calc(100vh-2rem)] flex flex-col animate-fade-in">
      <div className="flex items-center justify-between mb-6 flex-shrink-0">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Policy Management</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Upload and manage your company policies</p>
        </div>
      </div>

      <div className="flex flex-1 gap-6 overflow-hidden">
        {/* Left: Upload Policy Text - Fixed/Non-scrollable container */}
        <div className="w-1/2 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-card p-6 flex flex-col h-full">
          <div className="flex items-center gap-3 mb-4 flex-shrink-0">
            <div className="w-10 h-10 bg-brand/10 dark:bg-blue-900/30 rounded-full flex items-center justify-center">
              <Upload size={20} className="text-brand dark:text-blue-400" />
            </div>
            <div>
              <h2 className="font-semibold text-gray-900 dark:text-white">Upload Policy</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">Enter company policy text</p>
            </div>
          </div>
...
- Alcohol expenses require manager approval...`}
                className="w-full h-full p-4 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-brand dark:focus:ring-blue-500 focus:border-transparent text-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500"
              />
            </div>
            ...
            <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700 flex-shrink-0">
              ...
              <button
                ...
                className="px-4 py-3 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Clear
              </button>
            </div>
        ...
        <div className="w-1/2 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-card p-6 flex flex-col h-full">
          <div className="flex items-center gap-3 mb-4 flex-shrink-0">
            ...
            <div>
              <h2 className="font-semibold text-gray-900 dark:text-white">Current Policies</h2>
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
            {policies.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-12">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm">No policies uploaded yet</p>
                <p className="text-gray-400 dark:text-gray-500 text-xs mt-1">Upload a policy on the left to get started</p>
              </div>
            ) : (
              policies.map((policy) => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-lg border transition-colors ${
                      isActive 
                        ? 'bg-brand/5 dark:bg-blue-900/20 border-brand dark:border-blue-500 shadow-sm' 
                  ...
                  >
                    ...
                    {policy.summary && (
                      <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
                        ...
                        <p className="text-xs text-gray-600 dark:text-gray-300">{policy.summary}</p>
                      </div>
                    )}
                    <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-600 flex gap-2">
                      {!isActive && (
                        <button
                          ...
                          className="flex-1 px-3 py-2 text-xs font-medium text-brand dark:text-blue-400 bg-white dark:bg-gray-800 border border-brand dark:border-blue-500 rounded-lg hover:bg-brand/5 dark:hover:bg-blue-900/20 transition-colors"
                        >
                          Set as Active
                        </button>
                      )}
                      {isActive && (
                        <div className="flex-1 px-3 py-2 text-xs font-medium text-brand dark:text-blue-400 bg-brand/10 dark:bg-blue-900/30 rounded-lg text-center">
                          Currently Active
                        </div>
                      )}
                      <button
                        ...
                        disabled={isDeleting}
                        className="px-3 py-2 text-xs font-medium text-red-600 dark:text-red-400 bg-white dark:bg-gray-800 border border-red-200 dark:border-red-800 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
                      >
                        <Trash2 size={14} />
